...
function install_needed_packages {
    ...
    # sudo apt install libapr1-dev -y # INSTALLED FROM SOURCE
    # sudo apt install libaprutil1-dev -y # INSTALLED FROM SOURCE
...
function install_needed_packages_for_apache {
    echo "Installing needed packages"
    sudo apt-get update
    sudo apt install bzip2 -y
    # sudo apt install libapr1-dev -y # INSTALLED FROM SOURCE
...
function install_apr_util {
    ...
    ./configure --prefix="$APR_UTIL_FILE_INSTALL_LOCATION"
    make -j "$(nproc)" # nproc - print the number of processing units available
    make install -j "$(nproc)"
...
install_needed_packages_for_apache
#install_apr
install_apr_util
install_apache
#install_php
